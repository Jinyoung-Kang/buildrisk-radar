  ...
  return (
    <Layout title="대시보드">
      <PageTitle title="건설사 재무 × 지역 주택시장 조기경보"
        sub={<>건설업 상장사 {int(f.universe)}곳의 재무·공시(원문 구조화 포함)와 시군구 {int(f.regions)}곳의 미분양·가격지수·실거래를 규칙으로 점검합니다.</>} />
              ...
              <ul className="grid sm:grid-cols-2 gap-x-6 text-sm">
                {(data.jobOrder as string[]).map((j) => (
                  <li key={j} className="flex justify-between py-1.5 border-b border-line/60">
                    <span className="truncate" title={j}>{(data.jobTitles as Record<string, string> | undefined)?.[j] ?? j}</span>
